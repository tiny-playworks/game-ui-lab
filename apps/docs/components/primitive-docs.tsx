...
const commonRows: ApiRow[] = [row("className", "自定义类名。", "Custom class name.", "string", "-")];

const classAndStyleRows: ApiRow[] = [...commonRows, row("style", "内联样式。", "Inline styles.", "CSSProperties", "-")];
...
const docs: Record<PrimitiveId, PrimitiveDoc> = {
  "ability-bar": {
    ...
    api: [
      ...
      row(
        "active / variant",
        "技能激活态与类型。",
        "Active state and ability variant.",
        "boolean / 'basic' | 'ultimate' | 'passive'",
        "-",
      ),
      row("triggerKey", "键盘触发键。", "Keyboard trigger key.", "string", "-"),
      row(
        "resourceCost / comboHint / cooldownText",
        "消耗、连击提示与冷却文案。",
        "Cost, combo hint, and cooldown text.",
        "ReactNode",
        "-",
      ),
      row(
        "renderAbility",
        "自定义技能项渲染。",
        "Custom ability renderer.",
        collectionRendererType.replace("<T>", "<AbilityBarItem>"),
        "-",
      ),
  ...
  "target-frame": {
    ...
    api: [
      ...
      row(
        "elite / threat / weakness",
        "精英、威胁和弱点信息。",
        "Elite, threat, and weakness metadata.",
        "boolean / string",
        "-",
      ),
  ...
  "mini-map": {
    ...
    api: [
      ...
      row(
        "scanRadius / playerHeading / zoomLabel",
        "扫描圈、玩家朝向和缩放标签。",
        "Scan radius, player heading, and zoom label.",
        "number / string",
        "-",
      ),
  ...
  "dialogue-box": {
    ...
    api: [
      ...
      row(
        "source / typing / onAdvance",
        "来源、打字中状态和继续回调。",
        "Source, typing state, and advance callback.",
        "ReactNode / boolean / () => void",
        "-",
      ),
      ...commonRows,
    ],
  ...
  "inventory-grid": {
    ...
    api: [
      ...
      row(
        "slotType / stackCount / compareState",
        "格子类型、堆叠数和装备对比状态。",
        "Slot type, stack count, and comparison state.",
        "string / number",
        "-",
      ),
      row(
        "quickAction / onContextAction",
        "快捷动作和上下文回调。",
        "Quick action and context callback.",
        "ReactNode / (id, action, slot) => void",
        "-",
      ),
  ...
  "game-timer": {
    ...
    api: [
      ...
      row(
        "warningThreshold",
        "进入警告态的剩余比例阈值。",
        "Remaining ratio threshold for warning state.",
        "number",
        "-",
      ),
      ...classAndStyleRows,
  ...
  "shop-panel": {
    ...
    api: [
      ...
      row(
        "stock / discount / unavailableReason / details",
        "库存、折扣、不可购买原因和详情。",
        "Stock, discount, unavailable reason, and item details.",
        "ReactNode / number",
        "-",
      ),
  ...
  "chat-feed": {
    ...
    api: [
      ...
      row(
        "timestamp / channel / highlighted",
        "时间、频道和高亮状态。",
        "Timestamp, channel, and highlighted state.",
        "ReactNode / boolean",
        "-",
      ),
  ...
  return [
    {
      ...
      description: "Safer, fewer rewards",
      cost: "1 key",
      resultPreview: "Avoids patrol",
    },
    {
      id: "right",
      label: "Force the breach",
      description: "Risky but faster",
      cost: "20 stamina",
      resultPreview: "Calls reinforcements",
    },
  ];
}
...
function MiniMapPreview() {
  ...
  return (
    <MiniMap
      ...
      zones={[
        { id: "patrol", x: 58, y: 42, width: 24, height: 18, tone: "danger", label: isZh ? "巡逻区" : "Patrol zone" },
      ]}
      paths={[
        {
          id: "route",
  ...
  return (
    <RarityBorder tone="legendary">
      <div className="docs-rarity-sample">
        ...
        <Localized as="span" zh="适合把奖励结果收在一个明确容器里。" en="Wrap reward results in a clear container." />
      </div>
    </RarityBorder>
  );
}
  ...
  return (
    <ChatFeed
      messages={[
        {
          id: "1",
          author: isZh ? "系统" : "System",
          text: isZh ? "Boss 已进入战斗" : "Boss engaged",
          tone: "combat",
          timestamp: "12:04",
          channel: isZh ? "团队" : "Raid",
          highlighted: true,
        },
        {
          id: "2",
          author: isZh ? "队友" : "Ally",
          text: isZh ? "左侧通道更安全" : "Left path is safer",
          tone: "party",
          timestamp: "12:05",
          channel: isZh ? "队伍" : "Party",
        },
      ]}
    />
  );
