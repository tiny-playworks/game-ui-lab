...
const initialParty = [
  {
    ...
    maxHealth: 320,
    shield: 24,
    status: { id: "haste", label: "Haste", tone: "buff" as const, duration: "6s" },
  },
...
const mapZones = [{ id: "patrol", x: 58, y: 42, width: 24, height: 18, tone: "danger" as const, label: "Patrol zone" }];
  ...
  return (
    <section className="encounter-demo">
      ...
      <div className="encounter-demo__controls">
        <button type="button" onClick={() => handleAbility("strike")}>
          {isZh ? "攻击" : "Attack"}
        </button>
        <button type="button" onClick={() => handleAbility("burst")}>
          {isZh ? "爆发" : "Burst"}
        </button>
        ...
        <button type="button" onClick={openQuestLog}>
          {isZh ? "任务日志" : "Quest log"}
        </button>
        <button type="button" onClick={showBranchChoices}>
          {isZh ? "分支选项" : "Choices"}
        </button>
