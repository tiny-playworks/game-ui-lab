import {
  gameUiTokenGroups,
  gameUiTokens,
  type GameUiTokenOverrides,
  type GameUiThemeName,
    ...
    <>
      <section className="docs-section">
        ...
        <div className="docs-token-compare">
          ...
          <TokenScene
            title={isZh ? "Arcade + 局部 token" : "Arcade + scoped tokens"}
            theme="arcade"
            tokens={customTokens}
          />
        </div>
  ...
  return (
    <GameUiProvider className="docs-token-scene" theme={theme} tokens={tokens}>
      ...
      <div className="docs-token-scene__loot">
        <LootCard
          className="docs-token-scene__loot-item"
          name="星核"
          rarity="legendary"
          quantity={1}
          value="999"
        ...
        />
        <FloatingToast
          className="docs-token-scene__toast"
          title="Token Applied"
          message="Color, border, glow"
          variant="loot"
        />
      </div>
    </GameUiProvider>
...
function TokenSample({ kind, value }: { kind: string; value: string }) {
  ...
  if (kind === "text") {
    return (
      <span className="docs-token-sample docs-token-sample--text" style={{ color: value }}>
        Aa
      </span>
    );
  }
  ...
  return (
    <span
      className="docs-token-sample docs-token-sample--swatch"
      style={{ background: value, color: gameUiTokens.text }}
    />
  );
}
